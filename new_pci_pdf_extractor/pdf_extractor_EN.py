...
class PDFExtractor:
    ...
    def clean_text(self, text: str) -> str:
        ...
        text = re.sub(r'PCI DSS v4\.0\.1 SAQ D\s+for Merchants\s*,\s*Section 2:\s*Self\s*-Assessment Questionnaire\s+October\s+2024\s*\n', '', text)
        text = re.sub(r'©\s*2006−2024\s+PCI Security Standards Council\s*,\s*LLC\.\s*All Rights Reserved\.\s*Page\s+\d+\s*', '', text)

        # Remove specific header elements line by line
        text = re.sub(r'Section 2:\s*Self-Assessment Questionnaire D\s+for Merchants\s*\n', '', text)
        text = re.sub(r'Note:\s*The following requirements mirror the requirements in the PCI DSS Requirements and Testing Procedures\s+document\.\s*\n', '', text)
        text = re.sub(r'Self-assessment completion date:\s*YYYY-MM-DD\s*\n', '', text)

        # Remove generic section titles
        text = re.sub(r'Build and Maintain a Secure Network and Systems\s*\n', '', text)
        text = re.sub(r'Protect Account Data\s*\n', '', text)
        text = re.sub(r'Maintain a Vulnerability Management Program\s*\n', '', text)
        text = re.sub(r'Implement Strong Access Control Measures\s*\n', '', text)
        text = re.sub(r'Requ?irement \d+:\s*[^\n]+\n', '', text)

        # Remove answer option blocks
        text = re.sub(r'PCI DSS Requirement\s+Expected Testing\s+Response♦\s*\n', '', text)
        text = re.sub(r'\(+Check one response for each requirement\s*\)\s*\n', '', text)
        text = re.sub(r'In Place\s+In Place\s*\n\s*with CCW\s+Not\s*\n\s*Applicable\s+Not Tested\s+Not in\s*\n\s*Place\s*\n', '', text)

        # Remove lines containing the ♦ symbol
        text = re.sub(r'.*♦.*\n?', '', text, flags=re.MULTILINE)

        # Remove "Applicability Notes (continued)" labels
        text = re.sub(r'Applicability Notes\s*\(continued\)\s*\n', '', text)
    ...
    def parse_requirements_data(self, text: str) -> List[Dict]:
        ...
        # The \.? consumes an optional trailing period after the ID
        id_pattern = r'^((?:A)?\d+\.\d+(?:\.\d+(?:\.\d+)?)?)\.?\s*'
    ...
    def is_valid_requirement_id(self, potential_id: str, remaining_content: str, current_id: str) -> bool:
        ...
        requirement_indicators = [
            r'is\s+stored',
            r'(?:is|are)\s+not\s+stored',
            r'(?:is|are)\s+encrypted',
            r'(?:is|are)\s+(?:managed|maintained|configured|defined|implemented|assigned|restricted)',
            r'must\s+',
            r'shall\s+',
            r'cannot\s+',
            r'All\s+\w+',
            r'The\s+\w+\s+(?:that|which)',
            r'Processes\s+and\s+mechanisms',
        ]
        ...
        # If content starts with or contains a bullet point right after (with optional spaces),
        # it's likely a test section, not a new requirement
        if remaining_content.lstrip().startswith('•'):
            return False

        # If the content starts with "at" or "to" (like "at 3.3.1.3"), it's a reference
    ...
    def extract_text_only(self, content: str) -> str:
        ...
        # Remove guidance (Applicability Notes)
        content = re.sub(r'Applicability Notes?\s*.*', '', content, flags=re.DOTALL | re.IGNORECASE).strip()

        # Remove tests (lists with • and specific verbs)
        test_verbs = ['Examine', 'Observe', 'Interview', 'Inspect', 'Review', 'Compare']
        test_pattern = r'(•\s*(?:' + '|'.join(test_verbs) + ').*?)(?=•\s*(?:' + '|'.join(test_verbs) + ')|Applicability Notes?|$)'
    ...
    def extract_tests_and_guidance(self, content: str) -> Dict:
        ...
        guidance_match = re.search(r'Applicability Notes?\s*(.*)', content, re.DOTALL | re.IGNORECASE)
        if guidance_match:
    ...
    def split_text_by_bullets(self, text: str) -> List[str]:
        ...
        # Pre-processing: ensure each bullet is on its own line
        # This handles cases where multiple bullets are on the same line
        text = re.sub(r'([.!?])\s+(•)', r'\1\n\2', text)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        pdf_folder = "."
        pdf_files = ['PCI-DSS-v4-0-1-SAQ-D-Merchant.pdf']
    ...
    for pdf_file in pdf_files:
        ...
        if requirements_data:
            ...
            print(f"✓ {len(requirements_data)} requirements extracted and saved in {output_csv}")
        else:
